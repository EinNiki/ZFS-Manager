  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
        <button className="btn btn-secondary" style={{ height: 26, padding: '0 10px', fontSize: 11 }}>Scrub</button>
        <button className="btn btn-secondary" style={{ height: 26, padding: '0 10px', fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    api.getMetricsHistory('1m').then(res => {
      const seen = new Map<string, any>();
      for (const m of (res.metrics || [])) {
        if (!seen.has(m.collected_at)) {
          seen.set(m.collected_at, {
            timestamp: new Date(m.collected_at).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit' }),
            alloc: m.alloc_gb,
          });
        }
      }
  ...
  const storageData = histData1m.length > 2 ? histData1m : historicalStats;
      ...
      case 'storage-timeline':
        return (
          <Panel
            ...
            sub={histData1m.length > 2 ? 'Last 30 days' : histData1d.length > 2 ? 'Last 24 hours' : 'Current session'}
          >
            <div style={{ padding: '16px 20px' }}>
              {storageData.length > 1 ? (
                <div style={{ height: 180 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={storageData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <defs>
                      ...
                      <YAxis axisLine={false} tickLine={false} tick={AXIS_TICK}
                        tickFormatter={v => v >= 1000 ? `${(v/1000).toFixed(1)}T` : v >= 1 ? `${v.toFixed(0)}G` : `${(v*1024).toFixed(0)}M`}
                        width={44} />
                      <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [v >= 1 ? `${v.toFixed(2)} GB` : `${(v*1024).toFixed(0)} MB`, 'Used']} />
                      <Area type="monotone" dataKey="alloc" stroke="#6366f1" fill="url(#dashAllocGrad)"
                        strokeWidth={1.5} dot={false} isAnimationActive={false} activeDot={{ r: 3, strokeWidth: 0 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div style={{ height: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'var(--font-ui)' }}>Collecting storage history…</span>
                </div>
              )}
            </div>
          </Panel>
      ...
      case 'activity-log':
        return logs.length > 0 ? (
          <Panel
            ...
            right={<span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)' }}>{logs.length} events</span>}
          >
            <div style={{ padding: '0 20px', maxHeight: 280, overflowY: 'auto' }} className="no-scrollbar">
              {logs.slice(0, 20).map((log, i) => <LogRow key={log.id || i} log={log} />)}
            </div>
          </Panel>
        ) : null;
